        ...
        for i in range(0, 27):
            for j in range(0, 3):
                if j == 0:
                    ...
                    ferst_date = worksheet.cell_value(i, j)
                    print (ferst_date)
                    start_date=dt.strptime(ferst_date, '%Y.%m.%d %H:%M:%S')
                    dt_start.append(start_date)
                elif j == 2:
                    two_date = worksheet.cell_value(i, j)
                    end_date = dt.strptime(two_date, '%Y.%m.%d %H:%M:%S')
...
#        weeks=0
 #   )
    ...
    while  a <= 92:
        for ppc in range(len(vitok)):
            print(dt_start[ppc])
            track_shape, time_m, Fd_m, R_0_m = create_orbital_track_shapefile_for_day(tle_1, tle_2, pos_t, dt_start[ppc], dt_end[ppc], delta, track_shape, a)
